...
class HybridWingDetector:
    ...
    def _clean_probability_mask(self, mask):
        ...
        # Fill holes
        cleaned = morphology.remove_small_holes(cleaned, 10000)
        
        # Gentle morphological operations
        cleaned = morphology.opening(cleaned, morphology.disk(3))
        cleaned = morphology.closing(cleaned, morphology.disk(8))
    ...
    def _create_sparse_trichome_mask(self, peaks, shape):
        ...
        # Gentle morphological operations
        mask = morphology.closing(mask, morphology.disk(10))
    ...
    def _create_dense_trichome_mask(self, peaks, shape):
        ...
        mask = morphology.closing(mask, morphology.disk(8))
    ...
    def _sparse_wing_cleanup(self, mask, prob_map, raw_img):
        ...
        if prob_map.shape[-1] >= 4:
            ...
            expanded = morphology.dilation(cleaned, morphology.disk(5))
    ...
    def _refine_hybrid_mask(self, hybrid_mask, prob_mask, trichome_mask):
        ...
        if hybrid_area < prob_area * 0.5:  # Hybrid lost more than 50%
            ...
            confident_prob = morphology.erosion(prob_mask, morphology.disk(3))
...
def enhanced_intervein_processing_with_trichome_wing(prob_map, raw_img, cfg, output_dir, basename, 
    ...
    barrier_mask = morphology.dilation(combined_vein_mask, 
...
class EnhancedInterveinDetectorWithPentagone:
    ...
    def _detect_wing_from_probabilities(self, prob_map):
        ...
        wing_mask = morphology.closing(wing_mask, morphology.disk(10))
    ...
    def create_virtual_boundary_veins(self, wing_mask, actual_vein_mask, vein_width=10):
        ...
        wing_boundary = morphology.erosion(wing_mask) ^ wing_mask
        thick_boundary = morphology.dilation(wing_boundary, morphology.disk(vein_width//2))
        # dilated_veins = morphology.dilation(actual_vein_mask, morphology.disk(vein_width))
    ...
    #     barrier_mask = morphology.dilation(vein_mask, morphology.disk(1))  # Smaller dilation
...
class EnhancedInterveinDetector:
    ...
    def _detect_wing_from_raw_improved(self, raw_img, prob_map=None):
        ...
        filled_edges = ndimage.fill_holes(edges)
        ...
        wing_mask = morphology.opening(wing_mask, morphology.disk(5))
        wing_mask = morphology.closing(wing_mask, morphology.disk(15))
...
class ImprovedWingVeinDetector:
    ...
    def _advanced_postprocess(self, vein_mask, vein_prob):
        ...
        closed = morphology.closing(cleaned, morphology.disk(2))
        ...
        dilated = morphology.dilation(skeleton, morphology.disk(self.vein_width_estimate // 4))
    ...
    def _ensure_vein_connectivity_improved(self, vein_mask, vein_prob):
        ...
        dilated_veins = morphology.dilation(major_veins, morphology.disk(3))
    ...
    def _create_intervein_regions(self, vein_mask, prob_map):
        ...
        dilated_veins = dilation(vein_mask, disk(self.vein_width_estimate))
...
class ImprovedRegionDetector:
    ...
    def create_enhanced_barrier_mask(self, vein_mask, wing_mask):
        ...
        barrier_base = morphology.dilation(
            vein_mask, morphology.disk(self.config.vein_width_estimate)
        )
        
        # Add wing boundary as barrier but with smaller buffer
        wing_boundary = morphology.erosion(wing_mask, morphology.disk(3)) ^ wing_mask
        boundary_barrier = morphology.dilation(wing_boundary, morphology.disk(2))
    ...
    def recover_marginal_regions(self, enhanced_prob, regions_multi, wing_mask, vein_mask):
        ...
        # Find wing margins
        wing_boundary = morphology.erosion(wing_mask, morphology.disk(5)) ^ wing_mask
        marginal_zone = morphology.dilation(wing_boundary, morphology.disk(20))
        ...
        connected_marginal = morphology.closing(
...
def plot_voronoi_with_veins_enhanced(valid_regions, region_peaks_dict, background_img, 
    ...
    for region in valid_regions:
        ...
        wing_boundary = morphology.erosion(labeled_regions > 0) ^ (labeled_regions > 0)
